...
const outputSection = document.querySelector("#carSales")

salesByWeek.forEach(sale => {
    console.log(sale.sales_agent.first_name)
    console.log(sale.sales_agent.last_name)
    const agent = sale.sales_agent.first_name + " " + sale.sales_agent.last_name
    outputSection.innerHTML += `<h3>${agent}</h3>`

    const keys = Object.keys(sale.vehicle)
    for(key of keys) {
        console.log(`${key}: ${sale.vehicle[key]}`)
        outputSection.innerHTML += `<div>${key}: ${sale.vehicle[key]}</div>`
    }

    console.log(sale.gross_profit)
    const profit = sale.gross_profit
    outputSection.innerHTML += `<h4>Profit: $${profit}</h4>`
})